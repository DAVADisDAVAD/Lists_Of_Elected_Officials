...
with open(f'Data\Members\Senate_List_Aggregate', 'r') as g:
    Senate_List_Aggregate = json.load(g)
    # print(Congress_List_Aggregate)

for Senate in Senate_List_Aggregate:
    SenateName = Senate[0]
    SenateName = SenateName.replace('"', '')
    SenateName = SenateName.replace("'", '')
    ...
    print(SenateName)
    with open(f"Data\Bills\Senate\\{SenateName}", 'w') as f:
