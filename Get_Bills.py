...
for Congressman in Congress_List_Aggregate:
    ...
    while i <= Number:
        ...
        for bill in bills:
            ...
            # Congress Number
            Congress_Number = link.split('/')
            # print(Congress_Number)
            Congress_Number = Congress_Number[2].replace('-', ' ')
            # print(Congress_Number)

            # Get bill name
            PREbilltitle = str(bill.find(class_='result-title'))
            # print(PREbilltitle)
            billtitle = PREbilltitle[PREbilltitle.find('>') + 1:PREbilltitle.find('</')]
            # print(billtitle)

            # Get bill sponsor and link for cosponsors
            PREbillsponsor = bill.find(class_='result-item')

            sponsor_links = PREbillsponsor.find('a')

            sponsor = str(sponsor_links)
            sponsor = sponsor[sponsor.find(">") + 1:sponsor.find("</a>")]
            print(sponsor)

            # Get Date Introduced
            date = bill.find(class_='result-item').text
            ...
            # Get committees
            PREdata = bill.find_all(class_="result-item")
            # print(PREdata)
            Committee = PREdata[1].text
            Committee = Committee[Committee.find(": ") + 2:]
            Committee = Committee.strip()
            # print(Committee)

            # Get latest action
            # PREdata = bill.find_all(class_="result-item")
            LatestAction = PREdata[2].text
            LatestAction = LatestAction.split(':')
            LatestAction = LatestAction[1].strip()
            # print(LatestAction)

            # Get all actions (skip till later if difficult)

            # Get tracker data

            # Make List
            ListEntry = [data, billname, link, Congress_Number, billtitle, sponsor, date, Committee,
                         LatestAction]

            Bill_List = Bill_List + [ListEntry]

    print(CongressmanName)
    with open(f'Data\Bills\\{CongressmanName}', 'w') as f:
