...
con = sqlite3.connect('../../Data_Bases/Congress_Members.db')
c = con.cursor()

c.execute("select *from Congress_Members ORDER BY Name;")
Congress_Members = (c.fetchall())
length = int(len(Congress_Members))

con.close()

for Congressman in Congress_Members:
    CongressmanName = Congressman[0]
    CongressmanName = CongressmanName.replace('"', '')
    timestamp = str(datetime.now.strftime("%Y%m%d_%H-%M-%S"))
    print(CongressmanName + ' ' + timestamp)

    # Open Connection to congressman's DB
    con2 = sqlite3.connect(f'Data_Bases/Congress/{CongressmanName}Bills.db')
    c2 = con2.cursor()

    # c2.execute("DROP TABLE CongressMember_Legislation_All")

    c2.execute("""CREATE TABLE Congress_Legislation_All (
               Data text,
               Bill_Name text,
               Link tex
    ...
    while i <= Number:
        ...
        for bill in bills:
            ...
            c2.execute("INSERT INTO Congress_Legislation_All VALUES(?, ?, ?)", (data, billname, link))
            con2.commit()
    c2.close()
